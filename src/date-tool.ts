 ...
 * ```ts
 * // -------- Global Import(全局引入)
 * const BootsJS = require('toolsjs.js'); // Node
 * import BootsJS from 'toolsjs.js' // Es6 Module
 * BootsJS.DateTool.dateFormater(new Date(),'YYYY-MM-DD HH:mm:ss');
 *
 * // -------- Import on Demand(按需引入)
 * const { DateTool } = require('toolsjs.js/date-tool'); // Node
 * import { DateTool } from 'toolsjs.js/date-tool' // Es6 Module
...
export class DateTool {
  ...
  static dateFormater(
    ...
    formater: string = "YYYY-MM-DD HH:mm:ss"
  ): String {
  ...
  static getDateDiff(
    ...
    if (end.getTime() < start.getTime())
      throw new Error("The start date should be less than the end date");
    const diffSecond = end.getSeconds() - start.getSeconds(),
      diffMinute =
        end.getMinutes() - start.getMinutes() - (diffSecond < 0 ? 1 : 0),
      ...
      diffYear =
        end.getFullYear() - start.getFullYear() - (diffMonth < 0 ? 1 : 0);
    switch (unit) {
      case DateUnitEnum.all:
        ...
        return {
          ...
          days:
            diffDay < 0
              ? diffDay + this.getDaysInMonth(endYear, endMonth)
              : diffDay,
